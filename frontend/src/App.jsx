...
import {Dialog, BriefForm, CopyButton, AssetView, Uploader, TextEditor, ProductGallery, VariantList, PublishQueue} from './components';
...
function Panel({c,selected,busy,references,onDirty,onError,onSaveBrief,onSaveTexts,onUpload,onTransition,onOpen,onGenerate,onGenerateVariants,onSaveVariant,onRefreshVariant,onPublishSlot,onReuse}){
  ...
  if(selected==='video')return <>{index>=3?<><div className="notice"><strong>Um vídeo por cor.</strong> Anexe todos os MP4 de 15s. Use a imagem da mesma cor como referência.</div>{service('video')}{c.variants?.length>0?<VariantList variants={c.variants} images={c.assets} videos={c.assets} onError={onError} focus="video" onUpload={onUpload} busy={busy} disabled={immutable} immutable={immutable}/>:<>{editor('video','Prompt de vídeo')}<AssetView asset={image} title="Imagem aprovada para anexar" compact/><Uploader kind="video" exists={!!video} busy={busy} disabled={immutable} onUpload={onUpload}/></>}<div className="notice">Exporte cada vídeo com 15 segundos em {c.generator==='flow'?'1080 × 1920':'720 × 1280'}.</div></>:<div className="notice">Aprove as imagens e conclua o roteiro antes de criar o vídeo.</div>}</>;
