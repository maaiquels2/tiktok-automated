...
import { Dialog, BriefForm, CopyButton, AssetView, Uploader, TextEditor, ProductGallery, VariantList, PublishQueue, VideoMixer, PerformancePanel, VideoTimelinePreview } from './components';
...
export default function App(){
  ...
  function savePerformance(payload){
    return run(()=>api(`/campaigns/${c.id}/performance`,{method:'POST',body:{...payload,version:c.version}}),'Métricas salvas.');
  }
  function generateInsights(payload){
    return run(()=>api(`/campaigns/${c.id}/insights`,{method:'POST',body:{...(payload||{}),version:c.version}}),'Insights gerados.');
  }
  function gotoScript(){
    /* Panel parent may setSelected via stage click; soft hint */
    try{document.querySelector('[data-stage="script"]')?.click();}catch(e){}
  }
...
function Panel({c, selected, busy, references, onDirty, onError, onSaveBrief, onSaveTexts, onUpload, onTransition, onOpen, onGenerate, onGenerateVariants, onSaveVariant, onRefreshVariant, onPublishSlot, onMixVideos, onSavePerformance, onGenerateInsights, onGotoScript, onReuse}){
  ...
  return <>{index<5?<div className="notice">Aprove o vídeo antes de preparar a publicação.</div>:<>{c.status==='video_approved'&&<button className="primary full" disabled={busy} onClick={()=>onTransition('ready_to_publish')}>Preparar publicação <ArrowRight size={17}/></button>}{index>=6&&<PublishQueue c={c} busy={busy} immutable={immutable} onError={onError} onOpen={onOpen} onPublishSlot={onPublishSlot} onRefreshVariant={onRefreshVariant}/>
            <PerformancePanel c={c} busy={busy} immutable={immutable} onError={onError} onSavePerformance={onSavePerformance} onGenerateInsights={onGenerateInsights} onRefreshVariant={onRefreshVariant} onGotoScript={onGotoScript}/>}{c.status==='video_approved'&&<div className="notice">Depois de preparar, você escolhe cada cor/produto para subir no Studio.</div>}</>}</>;
}
